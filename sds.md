...
#### 服务端
**PHP的原因**
1. 上手容易，语法容易掌握(服务端小组都是第一次接触后段)
2. 与我们所选择的的mySQL数据库有这较好的契合度
3. 拥有许多功能强大的与前端需求相契合的函数与库
4. 可以较为方便的获取前端发来的http请求
...
#### 服务器端
由于我们服务器端只需要按照前端的需求来设计出相应的api接口，所以我们服务端的构架就是通过前端的功能流程图，我们将服务端的相关api接口按照负责功能的不同来分类，所以我们将我们的服务端api接口分为以下几个部分：
1. data部分，用于处理我们后台管理系统来增删改查Jupiter相关数据。
2. experience部分，用于处理体验功能的api模块
3. signup部分，用于处理用户注册的api模块
4. login部分，用于处理用户登录的api模块
5. modify部分，用于处理修改密码的api模块
6. search部分，用于处理搜索相关体验的api模块
7. token部分，用于七牛云服务器图片文件上传的api模块
